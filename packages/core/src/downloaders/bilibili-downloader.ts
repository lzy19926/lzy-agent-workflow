...
export class BilibiliDownloader extends Downloader {
  ...
  async downloadAudio(
    ...
    const audioPath = path.join(outputDir, `${videoId}.${this.platformConfig.audioFormat}`);

    if (fs.existsSync(tempFile)) {
      if (this.platformConfig.audioFormat !== 'm4a' || !fs.existsSync(audioPath)) {
  ...
  async downloadVideo(videoUrl: string, outputDir?: string): Promise<string> {
    ...
    // 查找并转换格式（支持多种可能的文件名）
    const videoPath = path.join(outputDir, `${videoId}.${this.platformConfig.videoFormat}`);
    ...
    for (const file of files) {
      if (file.startsWith(videoId) && file.endsWith('.mp4') && file !== path.basename(videoPath)) {
  ...
  async downloadSubtitles(
    ...
    try {
      ...
      const subtitleFile = path.join(outputDir, `${videoId}.${detectedLang || 'zh'}.${ext}`);
  ...
  private parseSrtContent(srtContent: string, language: string): TranscriptResult | null {
    try {
  ...
  private parseJson3Subtitle(subtitleFile: string, language: string): TranscriptResult | null {
    try {
      const content = fs.readFileSync(subtitleFile, 'utf-8');
      ...
      for (const event of events) {
        ...
        const text = segs.map((seg: { utf8?: string }) => seg.utf8 || '').join('').trim();
  ...
  private async runYtDlp<T>(url: string, options: Record<string, unknown>): Promise<T> {
    try {
      // 使用 yt-dlp 获取视频信息
      const info = await this.ytDlp.getVideoInfo(url);
      return info as T;
    } catch (error) {
      console.error('yt-dlp 执行失败:', error instanceof Error ? error.message : error);
      throw error;
    }
  }
  ...
  private async downloadWithProgress(
    ...
    try {
      ...
      console.error('yt-dlp 下载失败:', error instanceof Error ? error.message : error);
      throw error;
    }
  }
  ...
  async downloadAll(
    ...
    if (this.platformConfig.downloadCover && audioResult.coverUrl) {
      const coverExt = path.extname(audioResult.coverUrl.split('?')[0]) || '.jpg';
      const coverPath = path.join(outputDir, `${audioResult.videoId}_cover${coverExt}`);
      if (!fs.existsSync(coverPath)) {
        const https = await import('https');
        const file = fs.createWriteStream(coverPath);
        await new Promise<void>((resolve, reject) => {
          https.get(audioResult.coverUrl!, (res) => {
            res.pipe(file);
            file.on('finish', () => {
              file.close();
              resolve();
