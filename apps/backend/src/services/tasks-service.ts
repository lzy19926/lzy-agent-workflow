...
function getConfig(): VideoParserConfig {
  ...
  return {
    oss: {
      ...
      accessKeyId:
        process.env.OSS_ACCESS_KEY_ID || configFromFile.oss?.accessKeyId || "",
      accessKeySecret:
        process.env.OSS_ACCESS_KEY_SECRET ||
        configFromFile.oss?.accessKeySecret ||
        "",
    ...
    qwen: {
      apiKey:
        ...
        configFromFile.qwen?.apiKey ||
        configFromFile.asr?.apiKey ||
        "",
      model: configFromFile.qwen?.model || "qwen3-max",
    },
    outputDir:
      process.env.OUTPUT_DIR ||
      configFromFile.outputDir ||
...
async function executeTask(task: Task) {
  ...
  try {
    ...
    sendSSE(task.id, {
      type: "progress",
      taskId: task.id,
      data: { progress: 10, status: "processing", message: "开始下载..." },
    })
    ...
    const audioResult = await downloadService.downloadAudio(
      task.videoUrl,
      taskDir
    )
    ...
    const uploadResult = await uploader.uploadFile(audioResult.filePath)
    const fileUrl = uploader.getSignatureUrl(uploadResult.objectName, 3600)
    task.progress = 60
    sendSSE(task.id, {
      type: "progress",
      taskId: task.id,
    ...
    const summaryData = summarizedResult.data as {
      summary?: string
      keyPoints?: string[]
...
export function getHealth() {
  return {
    ...
    tasks: {
      total: tasks.size,
      processing: Array.from(tasks.values()).filter(
